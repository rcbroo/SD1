...
export const AI_PROVIDER_TEMPLATES: AIProviderTemplate[] = [
  ...
  },
  {
    ...
    capabilities: ['text-generation', 'vision', 'code-generation', 'multimodal', 'function-calling'],
    pricing: {
      input: 0.0005,
      output: 0.0015,
      unit: '1K tokens'
...
export const searchProviders = (query: string) => {
  ...
  return AI_PROVIDER_TEMPLATES.filter(provider =>
...
export const getOptimalProvider = (requirements: {
  ...
  if (requirements.capability) {
    candidates = candidates.filter(p => 
      p.capabilities.includes(requirements.capability as any)
    )
  }
  ...
  return candidates.sort((a, b) => {
    const scoreA = (a.rating * 0.3) + (a.performance.reliability * 0.3) + ((100 - a.performance.latency / 10) * 0.2) + ((1 / a.pricing.output) * 0.2)
    const scoreB = (b.rating * 0.3) + (b.performance.reliability * 0.3) + ((100 - b.performance.latency / 10) * 0.2) + ((1 / b.pricing.output) * 0.2)
